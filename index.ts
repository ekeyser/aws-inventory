...
export class AwsInventory {
    ...
    obtainAccountNumber(region: string) {
    ...
    run(Account: string, region: string, strService: string, apiCall: string, svcCallsAll: string[]) {
        return new Promise((resolve, reject) => {
            ...
            const RETRIES = 2;
            let requestSender = (fName: string, retry: number = 0) => {
    ...
    inventory() {
        return new Promise(async (resolve) => {
            ...
            this.objGlobal = {};
            // let Account: GetCallerIdentityCommandOutput;
            ...
            Object.keys(this.calls).forEach((region) => {
                Object.keys(this.calls[region]).forEach((service) => {
                    this.calls[region][service].forEach(() => {
                        numCalls++;
                    });
                });
            });
            ...
            const Account = await this.obtainAccountNumber('us-east-1');
            ...
            Object.keys(this.calls).forEach((strRegion) => {
                ...
                arrServices.forEach((strService) => {
                    this.calls[strRegion][strService].forEach((apiCall) => {
                        if (Account.Account) arrRequests.push(this.run(Account.Account, strRegion, strService, apiCall, this.calls[strRegion][strService]));
                    });
                });
            });
